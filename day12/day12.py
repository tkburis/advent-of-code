...
def part1(adj):
    ...
    while (not q.empty()):
        ...
        for neighbour in adj[curr_node]:
            if neighbour == 'end':
                all_paths.append(curr_path+['end'])
...
def part2(adj):
    ...
    while (not q.empty()):
        ...
        can_twice = 2 not in set(curr_path.count(x) for x in set(curr_path) if x.islower())  # has a small cave already been visited twice?
        if can_twice:
